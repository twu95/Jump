...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int r, int c) {
        return isLegal(player) && exists(r, c)
            && player == get(r, c).getSide()
                || get(r, c).getSide() == WHITE;
    }
    ...
    final Side getWinner() {
        Side whoGoesNow = whoseMove();
        Side whoGoesNext = whoseMove().opposite();
        int totalSide = size() * size();
        if (numOfSide(whoGoesNow) == totalSide) {

            return whoGoesNow;
        } else if (numOfSide(whoGoesNext) == totalSide) {
            return whoGoesNext;
        }
        return null;
    }
    ...
    public String toString() {
        ...
        for (int i = 1; i <= size(); i++) {
            ...
            for (int j = 1; j <= size(); j++) {
                int numSpots = get(i, j).getSpots();
                String s = get(i, j).getSide().toString();
                switch (s) {
                case ("red"):
                    s = "r";
                    break;
                ...
                toReturn += Integer.toString(numSpots) + s + " ";
            }
            toReturn += "\n";
        }
        toReturn += "===";
